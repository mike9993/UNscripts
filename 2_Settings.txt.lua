-- ============================================================
--  UNScripts Settings Plugin  (Script 2)
--  Injects dedicated Settings overlay into Script 1's Base UI.
...
local CFG_EXT    = ".cfg"
local PLG_FILE   = "settings_plugin"
...
local function findUI()
    Gui = game:GetService("CoreGui"):FindFirstChild("UNScriptsInterface")
    if not Gui then Gui = LocalPlayer:WaitForChild("PlayerGui"):FindFirstChild("UNScriptsInterface") end
    ...
    for _, c in ipairs(Gui:GetChildren()) do
        if c:IsA("TextButton") and (c.Text == "UNScripts" or c.Text == "UNS" or c.Text == "U\nN\nS") then
            PillUI = c
            break
        end
    end
...
local function applyTheme()
    ...
    if _G.UNScripts and _G.UNScripts.ApplyTheme then
        _G.UNScripts.ApplyTheme(cfgLightMode)
    end
end
...
local function syncTransparencyToSecondary()
    pcall(function()
        local g = game:GetService("CoreGui"):FindFirstChild("UNScripts_Secondary_UI") or (LocalPlayer:FindFirstChild("PlayerGui") and LocalPlayer.PlayerGui:FindFirstChild("UNScripts_Secondary_UI"))
        if g then
            ...
            for _, c in ipairs(g:GetChildren()) do
                if c:IsA("TextButton") and (c.Text == "UNScripts Secondary" or c.Text == "U\nN\nS") then c.BackgroundTransparency = cfgTransparency; break end
            end
        end
    end)
end
...
local function injectSettings()
    if not findUI() then
        task.wait(1)
        if not findUI() then
            warn("[UNScripts Plugin] Base UI not found")
            return
        end
    end
    ...
    _G.UNScripts.Notify = notify
    _G.UNScripts.SetNotifCorner = applyNotifCorner
    ...
    local function findSectionTitle(obj, stopAt)
        local p = obj.Parent
        while p and p ~= stopAt do
            for _, child in ipairs(p:GetChildren()) do
                if child:IsA("Frame") then
                    local tl = child:FindFirstChildWhichIsA("TextLabel", true)
                    if tl then
                        local sn = tl:GetAttribute("SearchName")
    ...
    local function findTabButton(container)
        local sectionScroll = container and container.Parent
        if not (sectionScroll and sectionScroll:IsA("ScrollingFrame")) then return nil end
        local tabBar = nil
        for _, child in ipairs(MainPage:GetChildren()) do
            if child:IsA("Frame") then
                for _, btn in ipairs(child:GetChildren()) do
                    if btn:IsA("TextButton") then tabBar = child; break end
                end
            end
            if tabBar then break end
        end
        if not tabBar then return nil end
        local idx = 0
        for _, c in ipairs(sectionScroll:GetChildren()) do
            if c:IsA("Frame") then
                idx = idx + 1
                if c == container then
                    local bi = 0
    ...
    local function findScrollFrame(obj)
        local p = obj.Parent
        while p do
    ...
    local function navigateToItem(item)
        if not item or not item.obj or not item.obj.Parent then return end

        local inSettings = false
        local p = item.obj
        while p do
            if p == SetPage then inSettings = true; break end
            if p == MainPage then break end
            if p == Main then break end
            p = p.Parent
        end

        -- 1. Switch page
        if inSettings then
            if not SetPage.Visible then SetPage.Visible = true; MainPage.Visible = false end
            if item.scrollFrame == ScrollShortcuts then switchSetTab("Shortcuts")
            elseif item.scrollFrame == ScrollThemes then switchSetTab("Themes")
            elseif item.scrollFrame == ScrollSystem then switchSetTab("System") end
        else
            if not MainPage.Visible then SetPage.Visible = false; MainPage.Visible = true end
            if _G.UNScripts.switchTab and item.tabContainer then
                for name, container in pairs(_G.UNScripts.tabContainers) do
                    if container == item.tabContainer then
                        _G.UNScripts.switchTab(name)
                        break
                    end
                end
            end
        ...
        -- 2. Close all other sections, then expand target
        task.wait()
        if item.obj and item.obj.Parent then
            local function closeAllSections(parent)
                for _, child in ipairs(parent:GetChildren()) do
                    if child:IsA("Frame") then
                        local fn = _G.sectionToggles and _G.sectionToggles[child]
                        if fn then fn(false) end
                    end
                end
            end
            if inSettings and item.scrollFrame then
                closeAllSections(item.scrollFrame)
            elseif item.tabContainer then
                closeAllSections(item.tabContainer)
            end
            task.wait(0.15)

            local wrapper = item.obj.Parent.Parent
            if wrapper then
                local collapsed = wrapper.Size.Y.Offset <= 40
                if not collapsed then
                    for _, child in ipairs(wrapper:GetChildren()) do
                        if child:IsA("Frame") then
                            for _, sub in ipairs(child:GetChildren()) do
                                if sub:IsA("TextLabel") and sub.Text == "+" then collapsed = true; break end
                            end
                            break
                        end
                    end
                ...
                if collapsed then
                    local fn = _G.sectionToggles and _G.sectionToggles[wrapper]
                    if fn then
                        fn(true)
                    else
                        for _, child in ipairs(wrapper:GetChildren()) do
        ...
        -- 3. Scroll to center item
        if not (item.obj and item.obj.Parent) then return end
        local scroll = findScrollFrame(item.obj)
        if scroll then
            task.wait()
            local viewH = scroll.AbsoluteSize.Y
            local centerOff = viewH / 2 - item.obj.AbsoluteSize.Y / 2
            local curOff = item.obj.AbsolutePosition.Y - scroll.AbsolutePosition.Y
            local newPos = scroll.CanvasPosition.Y + curOff - centerOff
            scroll.CanvasPosition = Vector2.new(0, math.max(0, newPos))
        end

        -- 4. Flash item
        local oc, ot = item.obj.BackgroundColor3, item.obj.BackgroundTransparency
        item.obj.BackgroundColor3 = C.accent
        item.obj.BackgroundTransparency = 0.1
        task.wait(0.7)
        if item.obj and item.obj.Parent then
            TweenService:Create(item.obj, TweenInfo.new(0.5), {BackgroundColor3 = oc, BackgroundTransparency = ot}):Play()
        end
    end

    local recentSearches = {}
    local MAX_RECENT = 5
    local function addRecentSearch(query, item)
        if query == "" or not item then return end
        for i, r in ipairs(recentSearches) do
            if r.query == query then table.remove(recentSearches, i); break end
        end
        table.insert(recentSearches, 1, {query = query, item = item})
        while #recentSearches > MAX_RECENT do table.remove(recentSearches) end
    end
    ...
    local function openSearch()
        ...
        local box = Make("TextBox", {
            ...
            BorderSizePixel = 0, Text = "", PlaceholderText = "Search features...",
        ...
        local allResults = {}
        local function collectFrom(parent, isSettings)
            if not parent then return end
            for _, desc in ipairs(parent:GetDescendants()) do
                local name = desc:GetAttribute("SearchName")
                if name and name ~= "" and (desc:IsA("Frame") or desc:IsA("TextButton")) then
                    local pathParts = {}
                    local scrollFrame = nil
                    local tabContainer = nil
                    local sectionTitle = findSectionTitle(desc, parent)
                    if isSettings then
                        local p2 = desc.Parent
                        while p2 and p2 ~= parent do
                            if p2 == ScrollShortcuts then scrollFrame = p2; table.insert(pathParts, 1, "Shortcuts"); break
                            elseif p2 == ScrollThemes then scrollFrame = p2; table.insert(pathParts, 1, "Themes"); break
                            elseif p2 == ScrollSystem then scrollFrame = p2; table.insert(pathParts, 1, "System"); break end
                            p2 = p2.Parent
                        end
                    else
                        local p2 = desc.Parent
                        while p2 and p2 ~= parent do
                            if p2.Parent and p2.Parent:IsA("ScrollingFrame") then
                                tabContainer = p2
                                break
                            end
                            p2 = p2.Parent
                    ...
                    if sectionTitle ~= "Active" then
                        if sectionTitle then table.insert(pathParts, sectionTitle) end
                        table.insert(allResults, {
                            obj = desc, name = name, path = table.concat(pathParts, " > "),
                            isSettings = isSettings, scrollFrame = scrollFrame, tabContainer = tabContainer,
                        })
                    end
                end
            end
        end
        for _, s in ipairs({ScrollShortcuts, ScrollThemes, ScrollSystem}) do collectFrom(s, true) end
        if MainPage then collectFrom(MainPage, false) end
        ...
        local function renderResults(query)
            ...
            if query == "" then
                if #recentSearches > 0 then
                    Make("TextLabel", {
                        Size = ud2(1,0,0,24), BackgroundTransparency = 1,
                        Text = "Recent Searches", TextColor3 = C.textSec,
                        TextSize = 10, Font = bold, ZIndex = 202, Parent = resultsScroll,
                    })
                    for _, r in ipairs(recentSearches) do
                        local row = Make("Frame", {
                            Size = ud2(1,0,0,44), BackgroundColor3 = C.part_bg,
                            BackgroundTransparency = 0.2, BorderSizePixel = 0,
                            ZIndex = 202, Parent = resultsScroll,
                        })
                        Make("UICorner", {CornerRadius = ud(0,6), Parent = row})
                        Make("TextLabel", {
                            Size = ud2(1,-16,0,20), Position = ud2(0,10,0,4),
                            BackgroundTransparency = 1, Text = r.item.name,
                            TextColor3 = C.textPri, TextSize = 12, Font = semi,
                            TextXAlignment = Enum.TextXAlignment.Left, ZIndex = 203, Parent = row,
                        })
                        Make("TextLabel", {
                            Size = ud2(1,-16,0,16), Position = ud2(0,10,0,24),
                            BackgroundTransparency = 1, Text = r.item.path,
                            TextColor3 = C.textSec, TextSize = 10, Font = reg,
                            TextXAlignment = Enum.TextXAlignment.Left, ZIndex = 203, Parent = row,
                        })
                        local captured = r.item
                        ...
                        hit.MouseButton1Click:Connect(function()
                            closeSearch()
                            task.spawn(function() navigateToItem(captured) end)
                        end)
                    end
                else
                    Make("TextLabel", {
                        Size = ud2(1,0,0,36), BackgroundTransparency = 1,
                        Text = "No recent searches", TextColor3 = C.textSec,
            ...
            for _, item in ipairs(allResults) do
                if item.name:lower():find(q, 1, true) then
                    ...
                    Make("TextLabel", {
                        Size = ud2(1,-16,0,16), Position = ud2(0,10,0,24),
                        BackgroundTransparency = 1, Text = item.path,
                        TextColor3 = C.textSec, TextSize = 10, Font = reg,
                        TextXAlignment = Enum.TextXAlignment.Left, ZIndex = 203, Parent = row,
                    })
                    local hit = Make("TextButton", {
                    ...
                    local captured = item
                    hit.MouseButton1Click:Connect(function()
                        addRecentSearch(box.Text, captured)
                        closeSearch()
                        task.spawn(function() navigateToItem(captured) end)
                    end)
                end
    ...
    _G.UNS_ShortcutsSection = scSec
    _G.UNS_ShortcutsSectionObj = scObj
    ...
    function _G.UNS_AddShortcutPill(label, defaultKeyName, onKeyChanged)
    ...
    function _G.UNS_AddScriptShortcut(name, toggleFn, defaultKeys)
        ...
        if not cfg.scriptKeybinds[name] then
            cfg.scriptKeybinds[name] = defaultKeys or {mode = 1, key1 = nil, key2 = nil}
        end
    ...
    local lightModeToggle = makeToggle(modePill, (PART_H-18)/2, function(on)
        ...
        if _G.UNScripts_Secondary and _G.UNScripts_Secondary.onLightModeChange then _G.UNScripts_Secondary.onLightModeChange(on) end
        local secPath = CFG_FOLDER.."/".."settings_plugin_secondary"..CFG_EXT
        local secCfg = cfgSafely(readfile, secPath) and cfgSafely(function() return HttpService:JSONDecode(cfgSafely(readfile, secPath)) end) or {}
        if type(secCfg) ~= "table" then secCfg = {} end
        secCfg.lightMode = on
        cfgSafely(writefile, secPath, HttpService:JSONEncode(secCfg))
    end, cfgLightMode)
    _G.UNScripts.onLightModeChange = function(on)
    ...
    local function updSl(x)
        ...
        applyTransparency(f * 0.85); syncTransparencyToSecondary()
    end
    ...
    makeSetDropdown(splSec, splObj, "Collapse Button", {"Right", "Left"}, cfgCollapsePos, function(val)
        cfgCollapsePos = val
        saveSettings()
        if _G.UNScripts and _G.UNScripts.SetCollapsePosition then
            _G.UNScripts.SetCollapsePosition(val)
        end
    end)
    ...
    makeToggle(orientRow, (PART_H-18)/2, function(on)
        cfgPillVertical = on
        if _G.UNScripts then _G.UNScripts.PillVertical = on end
        if PillUI and PillUI.Visible then
            if on then PillUI.Size = PILL_V; PillUI.Text = "U\nN\nS"
            else PillUI.Size = PILL_H; PillUI.Text = "UNScripts" end
        end
        saveSettings()
    end, cfgPillVertical)
    ...
    if _G.UNScripts then
        _G.UNScripts.PillVertical = cfgPillVertical
        _G.UNScripts.SaveInventory = cfgSaveInventory
    end
    if PillUI and PillUI.Visible then
    ...
    local function doReset()
        ...
        local s = CFG_FOLDER.."/settings.cfg"; if cfgSafely(isfile, s) then cfgSafely(delfile, s) end
    ...
    makeToggle(svPill, (PART_H-18)/2, function(on)
        cfgSaveInventory = on
        if _G.UNScripts then _G.UNScripts.SaveInventory = on end
        saveSettings()
        ...
        if not listeningKey and input.UserInputType == Enum.UserInputType.Keyboard then
            if #activeKeybind == 2 then
                ...
                or (input.KeyCode == activeKeybind[1] and UserInputService:IsKeyDown(activeKeybind[2])) then
                    if _G.UNScripts and _G.UNScripts.ToggleUI then
                        _G.UNScripts.ToggleUI()
                    end
                end
            end
    ...
    if _G.UNScripts and _G.UNScripts.SetCollapsePosition then
        _G.UNScripts.SetCollapsePosition(cfgCollapsePos)
    end

    applyTheme()
    applyTransparency(cfgTransparency)
    print("[UNScripts Plugin] Settings overlay injected successfully")
end

local ok, err = pcall(injectSettings)
if not ok then
    warn("[UNScripts Plugin] Failed:", err)
end
